	...
	$(function(){
		...
		Base.post(Base.url.rootUrl + '/user/getUserAddress', {}, function(result){
			if(result){
				...
				//静默调用查找商户接口
				Base.post(Base.url.rootUrl + '/2/2100/getMerchantList', {city: result.city}, function(result){
					if(result.length > 0){
				...
				Base.post(Base.url.rootUrl + '/2/getProAndCity', {}, function(result){
					if(result.length > 0){
						...
						weui.picker(cache.locationArr, {className: 'custom-classname', defaultValue: [cache.locationArr[0].value, cache.locationArr[0].children[0].value], onChange: function(){}, onConfirm: function(value){
							...
							//静默调用查找商户接口
							Base.post(Base.url.rootUrl + '/2/2100/getMerchantList', {city: value[1]}, function(result){
								if(result.length > 0){
		...
		$('#merchantShortName').on('focusin', function(e){
			...
			if(cache.merchantArr.length == 0){
				...
				Base.post(Base.url.rootUrl + '/2/2100/getMerchantList', option, function(result){
					if(result.length > 0){
						$.each(result, function(i, n){
							cache.merchantArr.push({label: n.merchantShortName, value: (n.merchantShortName + '-' + n.merchantNo)});
						});
				...
				if(amt.length > 0){
					if(Validate.validateAmt(amt)){
						if(cache.planArr.length == 0){
							Base.post(Base.url.rootUrl + '/2100/getPlanListByMerchantNo', {merchantNo: merchantNo}, function(result){
								$.each(result, function(i, n){
		...
		$('#btn-apply').on('click', function(e){
			...
			var amt = $('#amt').val();
			if(amt.length == 0){
				Base.alertMsg('车辆估值不能为空！', null, function(){$('#amt').focus();});
				return false;
			}
			var planId = $('#planId').val();
			if(planId.length == 0){
			...
			//加载框开启
			Base.loadingOn();
			Base.post(Base.url.rootUrl + '/2100/generateOrder', {merchantNo: merchantNo, orderName: (projectName + projectDetail), orderAmt: amt, planId: planId, chargeWay: 0}, function(result){
				//跳转生成订单成功页面
