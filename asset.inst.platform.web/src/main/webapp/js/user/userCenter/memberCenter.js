...
require(['json2','jquery', 'base', 'formatter', 'wx'], function(JSON, $, Base, Formatter, wx){
	...
	$(function(){
		...
		$('.mortgage').click(function(e){
			$('#buyCarMask').hide();
    		$('.financingMaskContent').hide();
        	window.location.href = '../mortgage/mortgage.html';
		});
		//点击我要质押
		$('.pledge').click(function(e){
        	$('#buyCarMask').hide();
    		$('.financingMaskContent').hide();
        	window.location.href = '../pledge/toFinance.html';
		})
		//调用请求微信JSSDK权限接口
		Base.post(Base.url.rootUrl + '/2/getBBSJSConfig', {reqUrl: window.location.href}, function(result){
			//配置微信JSSDK
			wx.config({
			      debug: false, 
			      appId: result.appId, 
		...
		//关闭遮罩
		$('.buyCarClose').click(function(e){
			$('#buyCarMask').hide();
			$('.buyCarMaskContent').hide(); 
		});
		$('.financingClose').click(function(e){
			$('#buyCarMask').hide();
			$('.financingMaskContent').hide(); 
		});
		//点击二手车
		...
		//点击询价订单
		$('#enquiryOrders').click(function(e){
			window.location.href = '../pledge/enquiryOrders.html?regId=' + cache.regId;
		});
	})
});