...
require(['jquery', 'base', 'formatter', 'common', 'validate'], function($, Base, Formatter, Common, Validate){
    ...
    function initSmsTabBtn(){
    	...
    	$('.qd').click(function() {
    		Base.loadingOn();
    		//帮帮手短信通道‘2’
    		Base.post(Base.url.rootUrl + '/unIntcpt-req/2/verifyImageVerify/user', $('#regForm').serializeObject(), function(result){
    			Base.loadingOut();
    			hideNumVerTab();
    			//短信倒计时
    ...
    function submitRegForm(){
    	Base.post(Base.url.rootUrl + '/unIntcpt-req/2/regist/user', $('#regForm').serializeObject(), function(result){
			Base.loadingOut();
