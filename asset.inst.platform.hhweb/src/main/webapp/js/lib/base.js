...
define(['jquery', 'json2', 'cons', 'weui'], function($, JSON, CONS, weui){
	...
	var _this = {
		url: {
			...
			webUrl: CONS.protocol + '://' + CONS.hostname + ':' + CONS.port + '/asset.inst.platform.hhweb',
			imgUrl: CONS.protocol + '://' + CONS.hostname + ':' + CONS.port + '/images'
		},
		/** 网页跳转 **/
		...
		toLogin: function(){
			//跳转首页
			window.top.location = _this.url.webUrl + '/views/hhuser/login/login.html';
		},
		/*回退*/
		rollBack: function(){
			window.history.go(-1);
