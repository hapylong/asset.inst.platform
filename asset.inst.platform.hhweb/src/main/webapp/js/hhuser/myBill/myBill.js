...
require(['json2', 'jquery', 'base', 'formatter'], function(JSON, $, Base, Formatter){
	$(function(){
	    ...
	    function query(){
	        ...
	        Base.post(Base.url.rootUrl + '/pay/selectCurrBill/Huahua', {}, function(result){
	    ...
	    function showData(List){
	        if(List.length == 0){
	    		...
	    		for(var i=0;i<List.length;i++){
	                ...
	                List[i].orderName + "</span></div><div class='orderBottom'><table id='"+i+"'><tr class='tableTitle'><td>期数</td><td>本期应还(元)</td><td>最迟还款日</td><td>已还金额(元)</td><td>还款状态</td></tr>"+
	    			...
	    			for(var j=0;j<List[i].billList.length;j++){
	    				...
	    				case 1 :
	    					tableContent += "<tr class='tableContent'><td>"+List[i].billList[j].repayNo+"</td><td>"+Formatter.formatterMoney(List[i].billList[j].curRepayAmt)+"</td><td>"+Formatter.timeformatter2(List[i].billList[j].lastRepayDate)+
	        				"</td><td>"+Formatter.formatterMoney(List[i].billList[j].curRealRepayamt)+"</td><td>待还款</td></tr>";
	    					break;
	    				case 2 :
	    					tableContent += "<tr class='tableContent'><td>"+List[i].billList[j].repayNo+"</td><td>"+Formatter.formatterMoney(List[i].billList[j].curRepayAmt)+"</td><td>"+Formatter.timeformatter2(List[i].billList[j].lastRepayDate)+
	        				"</td><td>"+Formatter.formatterMoney(List[i].billList[j].curRealRepayamt)+"</td><td>部分还款</td></tr>";
	    					break;
	    				case 3 : 
	    					tableContent += "<tr class='tableContent'><td>"+List[i].billList[j].repayNo+"</td><td>"+Formatter.formatterMoney(List[i].billList[j].curRepayAmt)+"</td><td>"+Formatter.timeformatter2(List[i].billList[j].lastRepayDate)+
	        				"</td><td>"+Formatter.formatterMoney(List[i].billList[j].curRealRepayamt)+"</td><td>已还款</td></tr>";
	    					break;
	    				default :
	    					break; 
	    				} 
		    		...
		    		Base.post(Base.url.rootUrl + '/selectBills', option, function(result){
			        	...
			        	for(var i=0;i<result.length;i++){
                          switch (result[i].status){  
			    				...
			    				case 1 :
			    					moreTr += "<tr class='tableContent moreContent'><td>"+result[i].repayNo+"</td><td>"+Formatter.formatterMoney(result[i].curRepayAmt)+"</td><td>"+Formatter.timeformatter2(result[i].lastRepayDate)+
		        				    "</td><td>"+Formatter.formatterMoney(result[i].curRealRepayamt)+"</td><td>待还款</td></tr>";
			    					break;
			    				case 2 :
			    					moreTr += "<tr class='tableContent moreContent'><td>"+result[i].repayNo+"</td><td>"+Formatter.formatterMoney(result[i].curRepayAmt)+"</td><td>"+Formatter.timeformatter2(result[i].lastRepayDate)+
		        				    "</td><td>"+Formatter.formatterMoney(result[i].curRealRepayamt)+"</td><td>部分还款</td></tr>";
			    					break;
			    				case 3 : 
			    					moreTr += "<tr class='tableContent moreContent'><td>"+result[i].repayNo+"</td><td>"+Formatter.formatterMoney(result[i].curRepayAmt)+"</td><td>"+Formatter.timeformatter2(result[i].lastRepayDate)+
		        				    "</td><td>"+Formatter.formatterMoney(result[i].curRealRepayamt)+"</td><td>已还款</td></tr>";
			    					break;
			    				default :
			    					break; 
		    			   }
