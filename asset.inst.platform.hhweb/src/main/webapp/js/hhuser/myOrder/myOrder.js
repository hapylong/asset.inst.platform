...
require(['json2', 'jquery', 'base', 'formatter'], function(JSON, $, Base, Formatter){
	...
	$(function(){
	    ...
	    function query(pageNum,url){
	        ...
	        Base.post(Base.url.rootUrl + '/1/myOrder', option, function(result){
	        	 // 加载信息
	    ...
	    function showData(List,currentPage){
	        if(List.length == 0 && currentPage == 1){ 
	    		...
	    		for(var i=0;i<List.length;i++){
	                ...
	                case 0: 
	                	var showDataHtml = "";
	                    showDataHtml += "<div class='block' riskStatus="+List[i].riskStatus+" merchantNo="+List[i].merchantNo+"><div class='orderTop'><img src='../../../img/orderIcon.png' class='orderIcon'><span class='orderName'>"+
	                ...
	                case 5:
	                	var showDataHtml = "";
	                	showDataHtml += "<div class='block' riskStatus="+List[i].riskStatus+" merchantNo="+List[i].merchantNo+"><div class='orderTop'><img src='../../../img/orderIcon.png' class='orderIcon'><span class='orderName'>"+
	                    List[i].orderName + "</span><span class='state'>待确认</span></div><div class='orderBottom'><p class='orderNumber'>订单编号 : "+List[i].orderId+
	                    "</p><ul class='orderLeft'><li>订单金额</li><li>每期分摊</li><li>分期期数</li></ul><ul class='orderRight'><li>"+
	                    Formatter.formatterMoney(List[i].orderAmt)+"元</li><li>"+Formatter.formatterMoney(List[i].monthInterest)+"元</li><li>"+List[i].orderItems+"</li></ul></div></div>";
	                    $(".borderArea").append(showDataHtml);
	                    break;
		...
		$(".borderArea").delegate(".block","click",function(){
			  var riskStatus = $(this).attr('riskStatus');
			  ...
			  if(merchantNo == 'yianjia'){
				  var url = 'HousingOrderDetails.html?orderId=' + orderId + '&riskStatus=' + riskStatus;
				  window.location.href = url;
			  }else{
				  var url = 'orderDetails.html?orderId=' + orderId + '&riskStatus=' + riskStatus;
				  window.location.href = url;
			  }
		});
	});
