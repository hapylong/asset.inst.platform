	    ...
	    function showData(List){
	        if(List.length == 0){
	    		...
	    		for(var i=0;i<List.length;i++){
	                ...
	                List[i].orderName + "</span></div><div class='orderBottom'><table id='"+i+"'><tr class='tableTitle'><td></td><td>期数</td><td>本期应还(元)</td><td>最迟还款日</td><td style='position:relative;width:25%;padding-right:1rem;'>罚息<img src='../../../img/tips.png' class='tips'></td></tr>"+
	                "</table></div>";
	    			$(".borderArea").append(showDataHtml);
	    			var tableContent = '';
	    			for(var j=0;j<List[i].billList.length;j++){
	    				tableContent += "<tr class='tableContent'><td><div class='checkbox'><input type='checkbox' class='checkboxInput' id='"+(String(i) + String(List[i].billList[j].repayNo))+"' name='"+(String(i) + String(List[i].billList[j].repayNo))+"' regId='"+List[i].billList[j].regId+"' orderId='"+List[i].billList[j].orderId+"' repayNo='"+List[i].billList[j].repayNo+"' amt='"+List[i].billList[j].curRepayAmt+"'/><label for='"+(String(i) + List[i].billList[j].repayNo)+"'></label></div></td><td>"+List[i].billList[j].repayNo+"</td><td>"+Formatter.formatterMoney(List[i].billList[j].curRepayAmt)+"</td><td>"+Formatter.timeformatter2(List[i].billList[j].lastRepayDate)+
