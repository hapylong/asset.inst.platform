...
require(['json2','jquery', 'base', 'formatter','weui','validate'], function(JSON, $, Base, Formatter, WEUI,Validate){
	$(function(){
		...
		var preAmt = location.search.match(new RegExp("[\?\&]amt=([^\&]+)","i"))[1];//待支付账单金额
		var regId = location.search.match(new RegExp("[\?\&]regId=([^\&]+)","i"))[1];
		var repayNo = location.search.match(new RegExp("[\?\&]repayNo=([^\&]+)","i"))[1];
	        ...
	        $(".planContentItem").delegate(".bankCard","click",function(){
	        	...
	        	var option = {
	        			...
	        			"payList": [{
		        			...
		        			"sumAmt": preAmt,
		        			"repayList": [
		        			    {"regId": regId, "orderId": orderId, "repayNo": repayNo, "amt":preAmt}
		        			],
		        			"regId": regId
	        			}]
	        	}
	        	var data = JSON.stringify(option);
	        	url = Base.url.rootUrl + '/pay/break/amtrepay/11?data=' + data;
	        	window.location.href = url;
			});
			//使用新卡
			$(".useNewCard").click(function(){
				var url = '../aboutMe/addBankCard.html?toPaySplit=1';
				window.location.href = url;
				//添加成功跳回
			});
