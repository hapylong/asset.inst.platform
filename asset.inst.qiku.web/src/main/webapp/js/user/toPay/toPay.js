	    ...
	    function showData(List){
	        if(List.length == 0){
	    		...
	    		for(var i=0;i<List.length;i++){
	                ...
	                List[i].orderName + "</span></div><div class='orderBottom'><table id='"+i+"'><tr class='tableTitle'><td></td><td>期数</td><td>本期应付租金(元)</td><td>最迟付款日</td><td style='position:relative;width:25%;padding-right:1rem;'>违约金</td></tr>"+
	                "</table></div>";
	    			$(".borderArea").append(showDataHtml);
	    			var tableContent = '';
	    			for(var j=0;j<List[i].billList.length;j++){
	    				var amtShow=Subtr(List[i].billList[j].curRepayAmt, List[i].billList[j].curRealRepayamt);
	    				var amt=List[i].billList[j].curRepayAmt;
	    				tableContent += "<tr class='tableContent'><td><div class='checkbox'><input type='checkbox' class='checkboxInput' id='"+(String(i) + String(List[i].billList[j].repayNo))+"' name='"+(String(i) + String(List[i].billList[j].repayNo))+"' regId='"+List[i].billList[j].regId+"' orderId='"+List[i].billList[j].orderId+"' repayNo='"+List[i].billList[j].repayNo+"' amt='"+amtShow+"'/><label for='"+(String(i) + List[i].billList[j].repayNo)+"'></label></div></td><td>"+List[i].billList[j].repayNo+"</td><td>"+Formatter.formatterMoney(amt)+"</td><td>"+Formatter.timeformatter2(List[i].billList[j].lastRepayDate)+
    		 ...
    		 if(!$(this).hasClass("noTap")){
    			 if($(".amoutMoney").html() != 0.00){
 	 		        ...
 	 		        $(".planContentItem").delegate(".bankCard","click",function(){
 	 		        	...
 	 		        	//console.log(data);
 	 		        	url = Base.url.rootUrl + '/pay/xfAmountRepay/11?data=' + data;
