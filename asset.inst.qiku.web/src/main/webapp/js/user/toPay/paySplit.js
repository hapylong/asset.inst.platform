...
require(['json2','jquery', 'base', 'formatter','weui','validate'], function(JSON, $, Base, Formatter, WEUI,Validate){
	$(function(){
		...
		var preAmt;
		Base.post(Base.url.rootUrl + '/pay/getAdvanceAmtInfo', {'id':id}, function(result){
			preAmt = result;
		...
		//payList
		var mobileObject = localStorage.getItem("mobileObject");
		mobileObject = JSON.parse(mobileObject);
		//支付
		$(".pay").click(function(){
	        ...
	        $(".planContentItem").delegate(".bankCard","click",function(){
	        	...
	        	var option = {
	        			...
	        			'payList': [
		        	        {
		        	            "regId": mobileObject.regId,
		        	            "orderId": mobileObject.orderId,
		        	            "repayModel": "all",
		        	            "sumAmt": mobileObject.sumAmt,
		        	        }
		        	                ]
	        	};	        	
	        	var data = JSON.stringify(option);
	        	url = Base.url.rootUrl + '/pay/advanceSettlement/11?data=' + data;
	        	window.location.href = url;
			});
			//使用新卡
			$(".useNewCard").click(function(){
				var url = '../aboutMe/addBankCard.html?id=' + id+'&earlyPay=1';
				window.location.href = url;
				//添加成功跳回
			});
