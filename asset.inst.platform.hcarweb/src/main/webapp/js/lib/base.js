...
define(['jquery', 'json2', 'cons', 'weui'], function($, JSON, CONS, weui){
	...
	var _this = {
		url: {
			...
			webUrl: CONS.protocol + '://' + CONS.hostname + ':' + CONS.port + '/asset.inst.platform.web',
			hwebUrl: CONS.protocol + '://' + CONS.hostname + ':' + CONS.port + '/asset.inst.platform.hcarweb',
		...
		toLogin: function(){
			//跳转首页
			window.top.location = _this.url.hwebUrl + '/views/user/login/login.html';
		},
		/*回退*/
		rollBack: function(){
			window.history.go(-1);
