	...
	$(function(){
		...
		Base.post(Base.url.rootUrl + '/getOrderInfo', option, function(result){
			...
			if(orderInfo.preAmtStatus == 2){
				$(".toshow").show();
				var preAmt = parseFloat( parseFloat(Formatter.null2Zero(orderInfo.preAmt)) - parseFloat(Formatter.null2Zero(orderInfo.receivedPreAmt)) );
		...
		$(".pay").click(function(){
	        ...
	        $(".planContentItem").delegate(".bankCard","click",function(){
	        	...
	        	url = Base.url.rootUrl + '/pay/xfPreAmountRepay/4?data=' + data;
