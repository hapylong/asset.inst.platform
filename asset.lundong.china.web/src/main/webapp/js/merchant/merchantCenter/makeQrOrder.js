...
require(['jquery', 'base', 'formatter', 'weui', 'validate'], function($, Base, Formatter, weui, Validate){
	$(function(){
		...
		$('#btn-code').on('click', function(e){
			...
			//加载框开启
			Base.loadingOn();
			Base.post(Base.url.rootUrl + '/7/merchant/createQrCode', {projectName: projectName, projectDetail: projectDetail, amt: amt, planId: planId, remark: $.trim($('#remark').val())}, function(result){
				//跳转生成订单成功页面
