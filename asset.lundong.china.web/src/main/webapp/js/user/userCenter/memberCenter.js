...
require(['json2','jquery', 'base', 'formatter', 'wx'], function(JSON, $, Base, Formatter, wx){
	...
	$(function(){
		...
		Base.post(Base.url.rootUrl + '/3/queryRiskInfo', {}, function(result){
			...
			$('.t-font').html('尊敬的'+ cache.regId + ',您好');
			cache.isQuery = true;
			if(result.riskInfo){
				cache.isRisk = true;
            }
		...
		//调用请求微信JSSDK权限接口
		Base.post(Base.url.rootUrl + '/7/getBBSJSConfig', {reqUrl: window.location.href}, function(result){
			//配置微信JSSDK
			wx.config({
			      debug: false, 
			      appId: result.appId, 
		...
		$('#aboutMe').click(function(e){
			window.location.href = '../aboutMe/aboutMe.html?version=1.1';
		});
		//点击信用管理
