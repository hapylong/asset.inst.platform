	...
	$(function(){
		window.judge = function(){
			...
			if(bankType != '' && CardReg.test(bankCard) && phoneReg.test(phone)){
				$(".redBtns").css("background","#354384").removeAttr("disabled");
			}else{
		...
		$(function(){
			...
			//回显银行卡信息
			Base.post(Base.url.rootUrl + '/get_bankcard ', {}, function(result){
				if(result != '' && result != null){
					$('#bankType').val(result.bankname);
					$('#bankCard').val(result.bankcardno);
					$('#phone').val(result.bankmobile);
					$(".redBtns").css("background","#354384").removeAttr("disabled");
				}
	        })
			//获取银行卡类型
